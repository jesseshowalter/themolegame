...
/**
 * The public mission briefing every player sees while the host has the round's
 * mission open — the challenge description + objectives. When the host closes
 * the mission (or opens the quiz), realtime bounces players back to standby.
 */
  ...
  return (
    <TerminalChrome agentName={session?.name} avatarUrl={session?.avatar} status="BRIEFING">
      ...
      <div className="mole-screen">
        ...
        <h2 className="status-headline mission-title">{title || 'MISSION'}</h2>
        ...
        <p className="mono-dim mole-foot">Complete the mission, then return for the quiz.</p>
      </div>
    </TerminalChrome>
  );
}
